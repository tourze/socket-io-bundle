...
final class SocketTest extends AbstractEntityTestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->socket->setSessionId($this->sessionId);
        $this->socket->setSocketId($this->socketId);
    }
    ...
    public function testDeliveryOperations(): void
    {
        ...
        $delivery = new class extends Delivery {
            private int $setSocketCallCount = 0;
            ...
            {
                ++$this->setSocketCallCount;
            }

            public function getSetSocketCallCount(): int
            {
                return $this->setSocketCallCount;
            }
        };
        ...
        $this->assertEquals(1, $delivery->getSetSocketCallCount());
    }
    ...
    {
        yield 'namespace' => ['namespace', '/test-namespace'];
        yield 'clientId' => ['clientId', 'client-123'];
        yield 'handshake' => ['handshake', ['key' => 'value']];
        yield 'lastPingTime' => ['lastPingTime', new \DateTimeImmutable()];
        yield 'lastDeliverTime' => ['lastDeliverTime', new \DateTimeImmutable()];
        yield 'connected' => ['connected', false];
        yield 'transport' => ['transport', 'websocket'];
    }
}
